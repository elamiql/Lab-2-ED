...
class Tree {
  private:
    ...
    struct Node {
        ...
        Node(std::string id_, std::string title_, std::string isbn_, int year_,
             std::string lang_, std::string desc_, double rating_, int pages_,
             Node *p = nullptr);
    };
    ...
    void preOrder(Node *node,
                  std::vector<std::string> &result); // Recorrido PreOrder
    void deleteSubtree(Node *node); // Libera memoria de un subárbol
    void borrar_ratings(Node *node,
                        double r); // Filtro de eliminación por rating
    void
    precursores(Node *node,
                std::vector<std::string> &result); // Filtro analítico por año
  ...
  public:
    ...
    bool isEmpty() const; // Comprueba si el árbol no tiene elementos
    int size() const;     // Retorna la cantidad de libros actuales

    // Inserta un nuevo libro validando que no se supere el límite k de la raíz
    bool insertBook(std::string id, std::string title, std::string isbn,
                    int year, std::string lang, std::string desc, double rating,
                    int pages, const std::vector<SimilarBook> &similares);
    ...
    precursores(); // Retorna IDs con similares estrictamente posteriores
};
